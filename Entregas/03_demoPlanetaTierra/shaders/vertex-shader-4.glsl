        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));         
            ...
            if (textureColor.x < 0.4 && textureColor.y < 0.15 && textureColor.z < 0.5) {
                position += normal*sin(100.0*uv.x+time*30.0)*0.01;
                position += normal*sin(100.0*uv.y+time*30.0)*0.01;
            } else {
